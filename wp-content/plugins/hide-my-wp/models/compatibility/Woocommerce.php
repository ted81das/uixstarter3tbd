...
class HMWP_Models_Compatibility_Woocommerce extends HMWP_Models_Compatibility_Abstract {
	...
	public function hookBruteForce() {
		...
		if ( HMWP_Classes_Tools::getOption( 'brute_use_math' ) ) { //math recaptcha
			...
			add_action( 'woocommerce_login_form', array( $this, 'woocommerce_brute_recaptcha_form' ), 99 );

			if ( HMWP_Classes_Tools::getOption( 'hmwp_bruteforce_register' ) ) {
				add_action( 'woocommerce_register_form', array( $this, 'woocommerce_brute_recaptcha_form' ), 99 );
			}

			if ( HMWP_Classes_Tools::getOption( 'hmwp_bruteforce_lostpassword' ) ) {
				add_action( 'woocommerce_lostpassword_form', array( $this, 'woocommerce_brute_recaptcha_form' ), 99 );
			}

		} elseif ( HMWP_Classes_Tools::getOption( 'brute_use_captcha_v3' ) ) { //recaptcha v3

			add_action( 'woocommerce_login_form', array( $this, 'woocommerce_brute_recaptcha_form_v3' ), 99 );

			if ( HMWP_Classes_Tools::getOption( 'hmwp_bruteforce_lostpassword' ) ) {
				add_action( 'woocommerce_lostpassword_form', array( $this, 'woocommerce_brute_recaptcha_form_v3' ), 99 );
			}
		}
